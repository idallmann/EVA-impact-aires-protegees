...
In this document are performed and plotted the following descriptive statistics on the protected areas (PAs) funded by French Development Agency (AFD) :
...
data_distrib_ctry = data_stat_nodupl %>%
  ...
  summarize(n = n(),
            freq = round(n/nrow(data_stat_nodupl), 2)*100) %>%
  ungroup() 
...
hist_distrib_ctry_top_shr = ggplot(data_distrib_ctry_top, 
                     aes(x = reorder(country_en, -freq), y = freq, fill = freq)) %>%
+ geom_bar(width = 0.50, fill= "#3182BD", stat="identity") %>%
+ geom_text(aes(label = paste(n, "PAs"), y = freq), 
        vjust = -0.1, color="black",
        size=3.5) %>%
...
+ labs(title = "Distribution of protected areas by countries",
       subtitle = paste("Sample :", sum(data_distrib_ctry$n), "protected areas."),
         x = "Countries",
         y = "# of protected areas (%)") %>%
  + theme(legend.position = "bottom",
      legend.key = element_rect(fill = "white"),
...
+ labs(title = "Countries most targeted by AFD funded non-marine protected areas",
       subtitle = paste("Sample :", sum(data_distrib_ctry$n), "non-marine protected areas. Numbers indicated above bars."),
         x = "Countries",
         y = "Share of protected areas (%)") %>%
  + theme(legend.position = "bottom",
      legend.key = element_rect(fill = "white"),
...
+ geom_text(aes(label = n, y = n), 
        vjust = -0.1, color="black",
        size=3) %>%
+ labs(title = "Distribution of non-marine protected areas by countries",
       subtitle = paste("Sample :", sum(data_distrib_ctry$n), "non-marine protected areas."),
         x = "Countries",
         y = "# of protected areas") %>%
  + theme(legend.position = "bottom",
      legend.key = element_rect(fill = "white"),
...
+ geom_text(aes(label = n, y = n), 
        vjust = -0.1, color="black",
        size=3) %>%
+ labs(title = paste0("Distribution of non-marine protected areas by countries \n", roi),
       subtitle = paste("Sample :", sum(data_distrib_ctry$n), "non-marine protected areas."),
         x = "Countries",
         y = "# of protected areas") %>%
  + theme(legend.position = "bottom",
...
+ geom_text(aes(label = paste(n, "PAs"), y = freq), 
        vjust = -0.1, color="black",
        size=3.5) %>%
+ labs(title = "Regions funding most non-marine protected areas",
       subtitle = paste("Sample :", sum(data_distrib_reg$n), "non-marine protected areas. Numbers indicated above bars."),
         x = "",
         y = "Share of protected areas (%)") %>%
  + theme(legend.position = "bottom",
      legend.key = element_rect(fill = "white"),
...
data_distrib_reg_afd = data_stat_nodupl %>%
  ...
  group_by(region) %>%
  summarize(n_afd = n(),
            freq_afd = round(n/nrow(.), 3)*100) %>%
  ungroup()

data_distrib_reg_afd_top = data_distrib_reg %>%
  arrange(-freq_afd) %>%
  slice(1:5)
...
data_distrib_reg_wdpa = data_wdpa %>%
  ...
  group_by(region) %>%
  summarize(n_wdpa = n(),
            freq_wdpa = round(n/nrow(.), 3)*100) %>%
  ungroup()

data_distrib_reg_wdpa_top = data_distrib_reg %>%
  arrange(-freq_wdpa) %>%
  slice(1:5)

#Tehn create a plotting dataset with both WDPA and AFD distribution
data_distrib_reg = data_distrib_reg_afd %>%
  left_join(data_distrib_reg_wdpa, by = "region")

data_distrib_reg_top = data_distrib_reg_afd_top %>%
  left_join(data_distrib_reg_wdpa_top, by = "region")
...
fig_area_pa_fr = ggplot(subset(data_time_area, is.na(year_funding_first) == FALSE),
  ...
  + labs(title = "Evolution de la superficie financée par l'AFD",
         subtitle = paste("Echantillon :", sum(data_time_area$n, na.rm = TRUE), "aires protégées couvrant", format(sum(subset(data_time_area, is.na(year_funding_first) == FALSE)$tot_area_noint_km2), digits = 1, scientific = FALSE, big.mark = " "), "km²"),
         caption = paste0("Sur les ", sum(data_time_n$n), " aires protégées financées recensées, ", sum(subset(data_time_n, is.na(year_funding_first) == FALSE)$n, na.rm = TRUE), " ont une date de financement connue.\nSoit ", format(sum(subset(data_time_area, is.na(year_funding_first) == FALSE)$tot_area_noint_km2), digits = 1, scientific = FALSE, big.mark = " "), " km² sur ", format(sum(data_time_area$tot_area_noint_km2), digits = 1, scientific = FALSE, big.mark = " "), " km²."),
         x = "Année",
         y = "Superficie (km²)") %>%
  + theme(legend.position = "bottom",
      legend.key = element_rect(fill = "white"),
...
fig_area_pa_en = ggplot(subset(data_time_area, is.na(year_funding_first) == FALSE),
  ...
  + labs(title = "Evolution of AFD funded area",
         subtitle = paste("Sample :", sum(data_time_area$n, na.rm = TRUE), "protected areas covering", format(sum(subset(data_time_area, is.na(year_funding_first) == FALSE)$tot_area_noint_km2), digits = 1, scientific = FALSE, big.mark = " "), "km²"),
         caption = paste0("Out of ", sum(data_time_n$n), " known funded protected areas, ", sum(subset(data_time_n, is.na(year_funding_first) == FALSE)$n, na.rm = TRUE), " have a known funding year.\nIn terms of area, ", format(sum(subset(data_time_area, is.na(year_funding_first) == FALSE)$tot_area_noint_km2), digits = 1, scientific = FALSE, big.mark = " "), " km² out of ", format(sum(data_time_area$tot_area_noint_km2), digits = 1, scientific = FALSE, big.mark = " "), " km²."),
         x = "Year",
         y = "Area (km²)") %>%
  + theme(legend.position = "bottom",
      legend.key = element_rect(fill = "white"),
