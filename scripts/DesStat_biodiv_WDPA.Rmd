...
source("02_scripts/functions_DesStat_biodiv.R", encoding = "utf-8")
```
...
pa_shp = 
  #read_sf("00_data_raw/WDPA_SHP/BDD_SHP_nodupl.shp") %>%
  aws.s3::s3read_using(
  FUN = sf::read_sf,
  # Mettre les options de FUN ici
  object = "diffusion/data_tidy/BDD_SHP_nodupl_pub.gpkg",
  bucket = "vuillota",
  opts = list("region" = "")) %>%
  st_make_valid() %>%
...
pa_joint = fread("01_data_tidy/BDD_DesStat_nodupl.csv") %>%
...
# write_portfolio(pa_pfolio_forest,
#       dsn = "01_data_tidy/mapme_bio_data/pa_pfolio_forest.gpkg",
#       overwrite = FALSE)

pa_pfolio_forest = read_portfolio("01_data_tidy/mapme_bio_data/pa_pfolio_forest.gpkg")

##Unest the sf file into a classic data frame without geometry
#Note replace() could be used instead of wase_when, and filter instead of subset for the same result
data_pfolio_forest = unnest(pa_pfolio_forest, cols="treecover_area") %>%
  sf::st_drop_geometry() %>%
  dplyr::select(id_pr,nm_ap,drct,wdpaid,ann_c,years,treecover) %>%
  mutate(treecover = case_when(treecover == 0 ~ NA, TRUE ~ treecover)) %>%
  subset(!is.na(treecover))

# fwrite(data_pfolio_forest,
#        "01_data_tidy/mapme_bio_data/data_pfolio_forest.csv")

data_pfolio_forest = fread("01_data_tidy/mapme_bio_data/data_pfolio_forest.csv")
...
pa_pfolio_mang = read_portfolio("01_data_tidy/mapme_bio_data/pa_pfolio_mang.gpkg")
...
# fwrite(data_pfolio_mang,
#        "01_data_tidy/mapme_bio_data/data_pfolio_mang.csv")

data_pfolio_mang = fread("01_data_tidy/mapme_bio_data/data_pfolio_mang.csv")
...
#       dsn = "01_data_tidy/mapme_bio_data/pa_pfolio_emi.gpkg",
#       overwrite = FALSE)

pa_pfolio_emi = fread("01_data_tidy/mapme_bio_data/pa_pfolio_emi.gpkg")
...
#        "01_data_tidy/mapme_bio_data/data_pfolio_emi.csv")

data_pfolio_emi = fread("01_data_tidy/mapme_bio_data/data_pfolio_emi.csv")
...
data_stat_treeloss2 = data_pfolio_forest %>%
  group_by(nm_ap) %>% 
  #Comoute variation over time in each PA
  mutate(lag_treecov = lag(treecover),
         loss = ((treecover - lag_treecov)/lag_treecov)*100) %>%
