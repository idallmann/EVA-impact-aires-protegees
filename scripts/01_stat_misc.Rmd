...
data_fc_africa =  data_fc %>%
  ...
  left_join(select(data_pa_afd, c(iso3, n_afd, area_afd_km2)), by = "iso3") %>%
  left_join(select(data_pa_wdpa, c(iso3, n_wdpa, area_wdpa_km2)), by = "iso3")
```
...
tbl_fc_africa = data_fc_africa %>%
  ...
  select(c(country_en, iso3, n_afd, n_wdpa, area_afd_km2, area_wdpa_km2, fc_2010_per)) %>%
...
print(xtable(tbl_fc_africa, 
             type = "latex", 
             auto = T,
             caption = "Data source: Global Forest Watch"),
