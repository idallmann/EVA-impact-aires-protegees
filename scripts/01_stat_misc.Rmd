...
install.packages(c("stargazer", "janitor", "questionr", "countrycode", "WDI", "mapme.biodiversity","ggrepel","landscapemetrics"))
library(tidyverse)
library(stargazer)
library(dplyr)
library(sf)
...
#   #st_read("data_raw/wdpa/wdpa_shp_global_raw.gpkg") %>%
  s3read_using(sf::st_read,
              ...
              bucket = "projet-afd-eva-ap",
              opts = list("region" = "")) %>%
# st_read(dsn = "/home/onyxia/work/EVA-impact-aires-protegees/data/WDPA_WDOECM_Jun2024_Public.gdb")%>%
  st_drop_geometry() %>%
  clean_names() %>%
  ...
  !is.na(wdpaid) & iucn_cat == "Ia" ~ "Ia-Strict nature reserve",
  !is.na(wdpaid) & iucn_cat == "Ib" ~ "Ib-Wilderness area",
  !is.na(wdpaid) & iucn_cat == "II" ~ "II-National park",
  !is.na(wdpaid) & iucn_cat == "III" ~ "III-Natural monument or feature",
  !is.na(wdpaid) & iucn_cat == "IV" ~ "IV-Habitat or species management area",
  !is.na(wdpaid) & iucn_cat == "V" ~ "V-Protected landscape or seascape",
  !is.na(wdpaid) & iucn_cat == "VI" ~ "VI-Protected area with sust. use of nat. res.",
...
# Dataset for GIE chapter 

data_afd_gie <- data_stat_nodupl %>% dplyr::select(region,marine,wdpaid,area_km2,year_funding_first,iucn_des_en)


data_fapbm_gie<- data_pa_fapbm %>% dplyr::select(region,marine,wdpaid,area_km2,iucn_des_en)%>%
  mutate(year_funding_first=2005)


data_gie <- rbind(data_fapbm_gie, data_afd_gie)
...
iucn_cat_colors_en = c(
  ...
  "IV-Habitat or species management area" = "#AE642D", # Marron
  "V-Protected landscape or seascape" = "#80D0D0",   # Bleu
  "VI-Protected area with sust. use of nat. res." = "#94812B", # Orange
  "III-Natural monument or feature"="#842E1B",
  "II-National park" = "#228B22", # Vert
  "Ib-Wilderness area"="#DFAF2C",
  "Ia-Strict nature reserve" = "#18391E"              
)
...
eco_colors_fr=c("Terrestre"="#228B22",
                "Côtier"="#8B6C42",
                "Marin"="#26C4EC","Non reférencé"="#BBBBBB")

order_iucn_en <- c("Not categorized",               
  "Not referenced",  
  "VI-Protected area with sust. use of nat. res." , 
...
data_gie$iucn_des_en<- factor(data_gie$iucn_des_en, levels = order_iucn_en)

data_cat_iucn = data_gie %>%
  filter(marine %in% c(0,1),region==roi) %>%
  group_by(iucn_des_en) %>%
  #number of PAs per IUCN category
  summarize(n_iucn = n()) %>%
  ungroup() %>%
  #Frequency of IUCN categories
...
data_eco$ecosyst_fr = fct_recode(data_eco$marine, 
                                 ...
                                 "Côtier" = "1", 
                                 "Marin" = "2") %>%
  fct_explicit_na(na_level = "Non référencé")



data_eco_hist = data_eco %>%
  st_drop_geometry() %>%
  group_by(ecosyst_fr,ecosyst_en) %>%
  summarize(n = n(),
            freq = round(n/nrow(data_eco), 2)*100) %>%
  ungroup()
...
donut_eco_en <- ggplot(data_eco_hist, aes(x = "", y = n, fill = ecosyst_en)) +
  ...
  labs(title = "Proportion of protected areas by ecosystem type \n(excluding not referenced PAs)",
       subtitle = paste("Sample :", sum(data_eco_hist$n), "out of", nrow(data_gie), "protected areas"),
       x = "",
       y = "") +
   scale_fill_manual(name = "Ecosystème",
                      values = eco_colors_fr) +
  theme_void() +
  theme(
...
eco_colors_fr=c("Terrestre"="#228B22",
                "Côtier"="#8B6C42",
                "Marin"="#26C4EC","Non référencé"="#BBBBBB")
data_eco$ecosyst_fr = fct_recode(data_eco$marine, 
                                 "Terrestre" = "0", 
                                 "Côtier" = "1", 
                                 "Marin" = "2") %>%
...
#donut chart in share (in English)
donut_eco_fr <- ggplot(data_eco_hist, aes(x = "", y = n, fill = ecosyst_fr)) +
  geom_bar(width = 1, stat = "identity", color = "white") +
  geom_label(aes(x = 1.3, label = paste0(freq, "%")), 
             color = "black", position = position_stack(vjust = 0.55), 
             size = 4, show.legend = FALSE) +
...
tbl_area_gie = data_gie %>%
  st_drop_geometry() %>%
  filter(marine %in% c(0,1),area_km2>0,region==roi) %>%
  summarize(n = n(),
            tot = format(sum(area_km2), big.mark = ",", digits = 1, scientific = FALSE),
            min = format(min(area_km2), big.mark = ",", digits = 1, scientific = FALSE),
            max = format(max(area_km2), big.mark = ",", digits = 1, scientific = FALSE),
...
## Distribution 
### For GIE chapter 
The dataset used here gather PA in Africa of data_nofund_nodupl and PAs from the FAPBM 

```{r}
# In french
data_fapbm_distrib=data_fapbm %>% dplyr::select("name","wdpaid","region","iso3","marine","area_km2","country_fr","country_en") # for the GIE chapter we are adding FAPBM PAs
data_afd_distrib= data_afd %>% dplyr::select("name_pa","wdpaid","region","iso3","marine","area_km2","country_fr","country_en")%>% dplyr::rename(name=name_pa)

data_stat_nodupl =st_drop_geometry(rbind(data_fapbm_distrib,data_afd_distrib))


#Surface and number of PAs by country
data_distrib_ctry = data_stat_nodupl %>%
  filter(marine %in% c(0,1)& region==focus) %>%
...
# Top 5 pays en termes de nombre d'AP
data_distrib_ctry_top_n = data_distrib_ctry %>%
  arrange(-freq_n) %>%  # Trier par pourcentage d'AP
  slice(1:5)  # Sélectionner les 5 premiers pays

# Top 5 pays en termes de superficie d'AP
data_distrib_ctry_top_area = data_distrib_ctry %>%
  arrange(-freq_area) %>%  # Trier par pourcentage de superficie
  slice(1:5)  # Sélectionner les 5 premiers pays

colors <- c("Niger" = "#4C8C2E",        # Vert clair
            "Madagascar" = "#A0522D",  # Marron rougeâtre
            "Mozambique" = "yellow4",  # Vert moyen
            "Gabon" = "darkred",       # Marron foncé
            "Côte d’Ivoire" = "darkgreen", # Vert vif
            "Kenya" = "navajowhite3")       # Marron doux

	

## Nombre d'aires protégées (AP)
hist_distrib_ctry_top_n_shr = ggplot(data_distrib_ctry_top_n, 
                                     aes(x = reorder(country_en, -freq_n), y = freq_n, fill = country_en)) +
  geom_bar(width = 0.50, stat = "identity") +
  geom_text(aes(label = paste0(n, " APs")),   # Afficher uniquement le nombre absolu d'AP
            vjust = 1.5, color = "white", size = 2, fontface="bold") +  
  scale_fill_manual(values = colors) +  # Appliquer la palette de couleurs
  labs(title = "Countries most targeted by AFD funded, non-marine protected areas",
       subtitle = paste("Échantillon :", sum(data_distrib_ctry$n), "aires protégées non-marines couvrant", 
                        format(sum(data_distrib_ctry$area_km2), digits = 2, scientific = TRUE), "km²"),
       x = "", y = "Part des aires protégées (%)") +
...
hist_distrib_ctry_top_area_shr = ggplot(data_distrib_ctry_top_area, 
                                        aes(x = reorder(country_en, -freq_area), y = freq_area, fill = country_en)) +
  geom_bar(width = 0.50, stat = "identity") +
  geom_text(aes(label = paste0(format(area_km2, digits = 2, scientific = TRUE), " km²")),  # Afficher uniquement la superficie absolue en km²
            vjust = 1.5, color = "white", size = 2, fontface="bold") +  
  scale_fill_manual(values = colors) +  # Appliquer la même palette de couleurs
  labs(title = "Countries most targeted by AFD funded, non-marine protected areas (area)",
       subtitle = paste("Échantillon :", sum(data_distrib_ctry$n), "aires protégées non-marines couvrant", 
                        format(sum(data_distrib_ctry$area_km2), digits = 2, scientific = TRUE), "km²"),
       x = "", y = "Part de la superficie financée (%)") +
  theme(legend.position = "bottom",
        legend.key = element_rect(fill = "white"),
        plot.title = element_text(size = 14, face = "bold"), 
        axis.text.x = element_text(angle = 0, size = 10, hjust = .5, vjust = .6),
...
## Number of PAs accross time 

```{r}
roi = "Africa"

data_time_range = data.frame(year = 
                              c(min(data_gie$year_funding_first, na.rm = TRUE):max(data_gie$year_funding_first, na.rm = TRUE))
)


data_time_n = data_gie %>%
...
Duplicate to correct init_portfolio issue 
...
data.fc = dl.fc %>%
  ...
  unnest(treecover_area) %>%
  group_by(ID_raw) %>%
 mutate(treecover_base2000 = (value - value[datetime== "2000-01-01"])/value[datetime== "2000-01-01"],
         treecover_ini = value[datetime== "2000-01-01"]) %>%
  ungroup() 
...
To delete once sure it's not needed
