...
list_iso_ie_africa_focus = unique(data_pa_ie_africa_afd$iso3)
list_pa_ie_africa_focus = unique(data_pa_ie_africa_afd$wdpaid)
list_iso = list_iso_ie_africa_focus
list_iso = c("SEN", "COM", "STP", "KEN", "GAB")
...
for (i in list_iso)
{
  ...
  for(j in list_pa_in)
  {
    ...
    df_fc_att_i = rbind(df_fc_att_i, df_att_m_j$df_fc_att)
    df_fl_att_i = rbind(df_fl_att_i, df_att_m_j$df_fl_att)
...
n_pa  = length(unique(df_plot_forest_loss$wdpaid))
  fct.labs <- c("Before Matching", "After Matching")
  names(fct.labs) <- c(FALSE, TRUE)
# n_focus = length(unique(data_pa_fapbm[data_pa_fapbm$wdpaid %in% unique(df_plot_forest_loss$wdpaid),]$wdpaid))
  n_nofocus  = length(unique(data_pa[data_pa$wdpaid %in% unique(df_plot_forest_loss$wdpaid) & !(data_pa$focus == T),]$wdpaid))
...
area_nofocus_ha = sum(data_pa[data_pa$wdpaid %in% unique(df_plot_forest_loss$wdpaid) & !(data_pafocus == T),]$area_km2, na.rm = TRUE) *100
...
fig_forest_loss_agg_tot_focus = ggplot(data = filter(df_plot_forest_loss_agg_focus, year == year.max),
    ...
    + labs(x = "",
           ...
           subtitle = paste0("Sample : protected areas of interest (" , n_fapbm, " areas covering ", format(area_fapbm_ha, big.mark = ","), "ha)"),
...
ggsave(paste(tmp, "fig_forest_loss_agg_tot_focus.png", sep = "/"),
       plot = fig_forest_loss_agg_tot_focus,
       device = "png",
       height = 6, width = 9)

ggsave(paste(tmp, "fig_forest_loss_agg_avg_nofocus.png", sep = "/"),
