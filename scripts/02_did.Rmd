...
install.packages(c("tictoc", "fixest", "cobalt", "future", "progressr", "did", "latex2exp", "janitor"))
...
data_fund_nodupl = data_fund %>%
  ...
  slice(1) %>%
  ungroup() %>%
  select(-c("mt_part_cofinancier_prevu_euro", "cofinanciers_siop")) %>%
...
fig_forest_loss_agg = ggplot(data = filter(df_plot_forest_loss_agg, year == year.max),
    ...
    + labs(x = "",
           ...
           subtitle = paste0("Sample : all protected areas in the analysis (" , n_pa, " areas covering ", format(area_tot_ha, big.mark = ","), "ha)"),
...
ggsave(paste(tmp, "fig_forest_loss_agg.png", sep = "/"),
       plot = fig_forest_loss_agg,
       device = "png",
       height = 6, width = 9)
...
list_wdpa_bad = c(109037, #not enough units for did computing
                  124355,
                  17367,
                  313046,
                  555651496,
                  555555500, #Kenya : issue while did computing
                  555555506,
                  303878)
...
fn_plot_att(df_fc_att = df_fc_att_tidy,
