...
for (i in list_iso)
{
  ...
  for(j in list_pa_i)
  {
    ...
    output_att_m_j = fn_did_att(iso = i, wdpaid = j, 
                          is_m = TRUE,
                      data_pa = data_pa,
                      alpha = 0.05,
                      load_dir = load_dir,
...
Some stats on the number of PAs analyzed. Check the number of pa analyzed eventually is coherent with the filter applied during the whole process.
...
df_plot_forest_loss_agg = df_plot_forest_loss %>%
  ...
  summarize(fc_tot_rel00_ha = sum(fc_tot_rel00_ha, na.rm = TRUE),
            fc_tot_rel00_ha_ci_lower = sum(fc_tot_rel00_ha_ci_lower, na.rm = TRUE),
            fc_tot_rel00_ha_ci_upper = sum(fc_tot_rel00_ha_ci_upper, na.rm = TRUE)) %>%
  ungroup() 

year.max = 2021
n_pa  = length(unique(df_plot_forest_loss$wdpaid))
area_tot_ha = sum(df_plot_forest_loss$area_ha, na.rm = TRUE)
fig_forest_loss_agg = ggplot(data = filter(df_plot_forest_loss_agg, year == year.max),
    ...
    + geom_errorbar(aes(ymax=abs(fc_tot_rel00_ha_ci_upper), ymin=abs(fc_tot_rel00_ha_ci_lower)), width=0.3, colour="grey70", alpha=0.9, size=1) %>%
    + geom_label(aes(label = format(round(abs(fc_tot_rel00_ha), 0), big.mark = ","), y = 0),
                 vjust = -0.5,
                 color = "white",
                 show.legend = FALSE) %>%
    ...
    + labs(x = "",
           ...
           subtitle = paste("All protected areas in the analysis :", n_pa, "areas covering", format(area_tot_ha, big.mark = ","), "ha")) %>%
    + facet_wrap(~matched,
...
Plot ATT for each PA

```{r}
list_ctry_plot = df_fc_att %>%
  select(iso3, wdpaid) %>%
  unique() %>%
  group_by(iso3, wdpaid) %>%
  summarize(time = c(5, 10)) %>%
  ungroup()

temp_fc = df_fc_att %>%
  select(c(region, iso3, wdpaid, treatment_year, time, year, att_per, cband_lower_per_95, cband_upper_per_95, att_pa, cband_lower_pa_95, cband_upper_pa_95)) %>%
  mutate(sig_pa = sign(cband_lower_pa_95) == sign(cband_upper_pa_95),
         sig_per = sign(cband_lower_per_95) == sign(cband_upper_per_95)) %>%
  filter(time %in% c(5, 10))

temp_fl = df_fl_att %>%
  select(c(region, iso3, wdpaid, treatment_year, time, year, att, cband_lower_95, cband_upper_95)) %>%
  mutate(sig = sign(cband_lower_95) == sign(cband_upper_95)) %>%
  filter(time %in% c(5, 10))

df_plot_fc_att = left_join(list_ctry_plot, temp_fc, by = c("iso3", "wdpaid", "time"))
df_plot_fl_att = left_join(list_ctry_plot, temp_fl, by = c("iso3", "wdpaid", "time"))

fig_att_per = ggplot(df_plot_fc_att, 
                    aes(x = att_per, y = iso3, 
                     xmin = cband_lower_per_95, xmax = cband_upper_per_95)) %>%
  + geom_errorbarh() %>%
  + theme_minimal()
fig_att_per
...
fig_att_pa = ggplot(df_plot_fc_att, 
                    aes(x = att_pa, y = iso3, 
                     xmin = cband_lower_pa_95, xmax = cband_upper_pa_95)) %>%
  + geom_errorbarh() %>%
  + theme_minimal()
fig_att_pa

fig_att_fl = ggplot(df_plot_fl_att, 
                    aes(x = att, y = as.factor(wdpaid), 
                     xmin = cband_lower_95, xmax = cband_upper_95,
                     group = sig)) %>%
  + geom_point(aes(color = sig)) %>%
  + geom_errorbarh(aes(color = sig)) %>% 
  + facet_wrap(time~., ncol = 2, #scales = 'free_x'
               ) %>%
  + labs(x = "Averae treatment effect (p.p)",
         y = "WDPA ID",
         title = "") %>%
  + theme_minimal()
plot(fig_att_fl)
