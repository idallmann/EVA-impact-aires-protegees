...
list_iso_ie_focus = unique(data_pa_ie_africa_focus$iso3)
list_pa_ie_focus = unique(data_pa_ie_africa_focus$wdpaid)
list_iso = list_iso_ie_focus
list_iso = c("COM", "STP", "GNB", "GMB", "SEN", "MAR", "GAB", "KEN", "CMR", "COG", "MDG", "MOZ", "NER", "TCD")
...
df_pa_overlap_ie_rm = df_pa_overlap_focus %>%
  filter(marine %in% c(0,1) & status_yr >= yr_min & area_km2 >= 1 & rm_rep == F & status %in% c("Proposed", "Designated", "Inscribed", "Established"))
df_pa_overlap_ie_rm_focus = df_pa_overlap_focus %>%
  filter(marine %in% c(0,1) & status_yr >= yr_min & area_km2 >= 1 & rm_rep == F & status %in% c("Proposed", "Designated", "Inscribed", "Established") & focus == T)


test = df_pa_overlap_ie_rm %>%
...
n_pa_pre_filt = length(unique(df_pa_overlap_ie$wdpaid))
n_ctry_pre_filt = length(unique(df_pa_overlap_ie$iso3))
#### Part due to filtering AND overlap removal
n_pa_pre_filt_over = length(unique(df_pa_overlap_ie_rm$wdpaid))
n_ctry_pre_filt_over = length(unique(df_pa_overlap_ie_rm$iso3))
### PA of interest
...
#### Part due to filtering
n_pa_pre_filt_focus = length(unique(df_pa_overlap_ie_focus$wdpaid))
n_ctry_pre_filt_focus = length(unique(df_pa_overlap_ie_focus$iso3))
#### Part due to filtering AND overlap removal
n_pa_pre_filt_over_focus = length(unique(df_pa_overlap_ie_rm_focus$wdpaid))
n_ctry_pre_filt_over_focus = length(unique(df_pa_overlap_ie_rm_focus$iso3))
...
On a map, we want to plot for each country the polygon of all PA (of interest and others), their estimated treatment effect (5 and 10 years) , their significance.
