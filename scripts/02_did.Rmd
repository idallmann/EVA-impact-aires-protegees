...
opts = list("region" = "")) %>%
  ...
  select(c(region, iso3, country_en, wdpaid, focus, fc_tot_ha))
...
### Losses related to overlap removal
...
df_pa_overlap_ie_rm = df_pa_overlap_focus %>%
  filter(marine %in% c(0,1) & status_yr >= yr_min & area_km2 >= 1)

#Finally we can assess the losses due to the overlap restricitoon
## In the sample, number of PA to remove 
n_pa_rm_poly = sum(df_pa_overlap$rm_poly)
## Among the PA we can analyse in the sample, the losses
n_pa_ie_rm_poly = sum(df_pa_overlap_ie_rm$rm_poly)
## Among the PA of interest we can analyse, the losses
n_pa_ie_focus_rm_poly = sum(filter(df_pa_overlap_ie_rm, focus == T)$rm_poly)
```
