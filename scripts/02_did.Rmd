...
#Initialize a dataframe to store pre-did dataframe of each PA in the country. Used later to aggregate results at country or world level
df_did = data.frame()
...
for (i in list_iso)
{
  ...
  #Initialize a dataframe to store pre-did dataframe of each PA in the country. Used later to aggregate results at country level
  df_did_i = data.frame()
  ...
  for(j in list_pa_i)
  {
    ...
    if(output_wolf_m_j$is_ok == FALSE) 
      {next} else df_fl_wolf_m_j = output_wolf_m_j$df_fl_wolf_m_j
    ...
    df_did_i = rbind(df_did_i, df_att_m_j$df_did)
    df_pre_test_i = rbind(df_pre_test_i, df_att_m_j$df_pre_test)
    ...
    #
    #if(output_att_m_j$is_ok == FALSE) {next} else df_att_m_j = output_att_m_j
    #df_pre_test = rbind(df_pre_test, df_att_m_j$df_pre_test)
  ...
  #Pre-did dataframe for country i
  ## Keep only one pixel-time observations. As PA in a country can have similar controls, there is a risk a control pixel appears more than once in df_did_i
  df_did_i_clean = df_did_i %>%
    group_by(year, assetid) %>%
    slice(1) %>% 
    ungroup()
  ## Store the country pre-did dataframe to df_did that gathers all country pre-did dataframes.
  df_did = rbind(df_did, df_did_i_clean)
  aws.s3::s3write_using(
  FUN = data.table::fwrite,
  df_did_i_clean,
  object = paste(save_dir, i, paste0("df_did_", i, ".csv"), sep = "/"),
  bucket = "projet-afd-eva-ap",
  opts = list("region" = ""))
...
#Save the pre-did dataframe of all country in a dataframe. Used for aggregating at country, region or world level
aws.s3::s3write_using(
FUN = data.table::fwrite,
df_did,
object = paste(save_dir, "df_did.csv", sep = "/"),
bucket = "projet-afd-eva-ap",
opts = list("region" = ""))
