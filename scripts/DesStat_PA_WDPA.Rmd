...
pie_cat_iucn = ggplot(data_cat_iucn, 
  ...
  + geom_label_repel(aes(x=1.2, label = round(freq_iucn, 1)), 
             color = "white", 
...
tbl_gov = data_stat_nodupl %>%
  ...
  mutate(n_tot = sum(n),
         freq = round(n/n_tot*100,3)) %>%
  select(-n_tot)
names(tbl_gov) = c("Governance","Number of PAs","Share of PAs (%)")

 # print(xtable(tbl_gov, type = "latex"), file = "DesStat/gouvernance/tbl_gov_en.tex")

tbl_gov_knwn = data_stat_nodupl %>%
  mutate(gov_type = case_when(gov_type == "" ~ "Not referenced",
                              TRUE ~ gov_type)) %>%
  ...
  mutate(n_tot = sum(n),
         freq = round(n/n_tot*100,3)) %>%
  select(-n_tot)
names(tbl_gov_knwn) = c("Governance","Number of PAs","Share of PAs (%)")

 # print(xtable(tbl_gov_knwn, type = "latex"), file = "DesStat/gouvernance/tbl_gov_knwn_en.tex")

pie_gov_knwn = 
  ggplot(tbl_gov_knwn, 
       aes(x="", y= `Share of PAs (%)`, fill= Governance)) %>%
  + geom_bar(width = 1, stat = "identity", color="white") %>%
  + geom_label(aes(x=1.3, label = `Share of PAs (%)`), 
               color = "black", 
               position = position_stack(vjust = 0.55), 
               size=2.5, show.legend = FALSE) %>%
  + coord_polar("y", start=0) %>%
