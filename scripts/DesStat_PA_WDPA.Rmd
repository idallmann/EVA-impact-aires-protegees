...
#PA identified with WDPAID
base_id = data_stat %>% filter(!is.na(wdpaid)) %>% group_by(wdpaid) %>% slice(1)
base_old_id = base_nodupl_old %>% filter(grepl("NA", wdpaid) == FALSE) 

#PA whose WDPAID is unknown
##For the BDD_joint, need to find a key to uniquely identify each PA (already done in base_old)
base_na = data_stat %>% filter(is.na(wdpaid))
base_old_na = base_nodupl_old %>% filter(grepl("NA", wdpaid) == TRUE) 


#Identify wdpaid in old not in new
base_wdpaid_old_only = base_old_id %>% subset(!(wdpaid %in% base_id$wdpaid))
```
...
pa_int = st_intersection(pa_shp_tidy, pa_shp_tidy) %>%
  ...
  select(wdpaid, iso3, drct, ann_c, wdpaid.1, iso3.1, drct.1, ann_c.1, geometry, area_int)
...
# ggsave(plot = pie_cat_iucn,
#        filename = "pie_cat_iucn.jpeg",
#        path = "DesStat/IUCN",
#        height = 6, width = 9)
...
# ggsave(plot = pie_cat_iucn_ref,
#        filename = "pie_cat_iucn_ref.jpeg",
#        path = "DesStat/IUCN",
#        height = 6, width = 9)
...
```{r}
data_distrib_dr = data_stat_nodupl %>%
  group_by(direction_regionale) %>%
  summarize(n = n(),
            freq = round(n/nrow(data_stat_nodupl), 3)*100) %>%
  ungroup() %>%
...
# print(xtable(tbl_distrib_area_no0, type = "latex"), file = "DesStat/surface/tbl_distrib_area_no0.tex")
```
...
# print(xtable(tbl_area_avg_ctry, type = "latex"), file = "DesStat/surface/tbl_area_avg_ctry.tex")
# print(xtable(tbl_area_avg_dr, type = "latex"), file = "DesStat/surface/tbl_area_avg_dr.tex")
...
# ggsave(plot = fig_area_pa,
#        filename = "fig_area_pa.png",
#        path = "DesStat/time_evolution",
#        height = 6, width = 9)
...
# ggsave(plot = fig_area_cum_pa,
#        filename = "fig_area_cum_pa.png",
#        path = "DesStat/time_evolution",
#        height = 6, width = 9)
...
data_stat_projet_nodupl_utf8 = data_stat_utf8 %>%
  dplyr::distinct(id_projet, .keep_all = TRUE)
...
#print(xtable(data_fund_per_area_dr, type = "latex"), file = "DesStat/funding/tbl_fund_per_area_dr.tex")
...
#print(xtable(tbl_fund_per_area_wld, type = "latex"), file = "DesStat/funding/tbl_fund_per_area_wld.tex")
...
# print(xtable(tbl_type_prod, type = "latex"), file = "DesStat/funding/tbl_fund_type.tex")
...
# print(xtable(tbl_fund_avg_type, type = "latex"), file = "DesStat/funding/tbl_fund_avg_type.tex")
```
...
names(tbl_gov_fr) = c("Gouvernance","Nombre d'AP","Proportion (%)")
  ...
  # print(xtable(tbl_gov_en, type = "latex"), file = "DesStat/gouvernance/tbl_gouv_en.tex")
  # print(xtable(tbl_gov_fr, type = "latex"), file = "DesStat/gouvernance/tbl_gouv_fr.tex")
...
# print(xtable(tbl_gov_knwn_en, type = "latex"), file = "DesStat/gouvernance/tbl_gov_knwn_en.tex")
# print(xtable(tbl_gov_knwn_fr, type = "latex"), file = "DesStat/gouvernance/tbl_gov_knwn_fr.tex")
...
# ggsave(plot = pie_gov_knwn_en,
#        filename = "pie_gov_knwn_en.png",
#        path = "DesStat/gouvernance",
#        height = 6, width = 9)
...
# ggsave(plot = pie_gov_knwn_fr,
#        filename = "pie_gov_knwn_fr.png",
#        path = "DesStat/gouvernance",
#        height = 6, width = 9)
```
