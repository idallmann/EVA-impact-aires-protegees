...
data_treecover_evo = data_treeloss %>%
...
fig_treeloss = ggplot(data = data_treecover_evo,
                      aes(x = year)) %>%
  + geom_line(aes(y = round(treecover_ha/treecover_2000_ha*100, 2)), color = "#3182BD") %>%
  + geom_point(aes(y = round(treecover_ha/treecover_2000_ha*100, 2)), color = "#3182BD") %>%
  + labs(x = "", y = "% of 2000 tree cover", title = "Evolution of forest cover in Madagascar",
...
-   Three groups : FAPBM, not FAPBM, all PAs

-   Compute evolution of forest cover with 2000 as base year

```{r}
#First try : export polygons of non-marine PAs in MDG and compute with GFW analysis tool (faster)
...
test = wdpa_mdg %>% slice(1)

data.tree = init_portfolio(wdpa_fapbm,
                     years = 2000:2022,
                     add_resources = FALSE) %>%
...
# wdpa_mdg,
# # Mettre les options de FUN ici
# object = "data_tidy/GFW/MDG/BDD_PA_MDG_GFW.geojson",
# bucket = "projet-afd-eva-ap",
# opts = list("region" = ""))
