...
data_plot_mdg = data_treeloss %>%
  ...
  mutate(cum_loss = cumsum(loss_ha),
         treecover_ha = treecover_2000_ha - cum_loss,
         treecover_rel00 = treecover_ha/treecover_2000_ha*100)

fig_treeloss_mdg = ggplot(data = data_plot_mdg,
                      aes(x = year, y = treecover_rel00)) %>%
...
data_plot_fapbm = data.tree.fapbm %>%
  ...
  pivot_longer(cols = c(starts_with("treecover")),
               names_to = c("var", "year"),
               names_sep = "_",
               values_to = "treecover_ha") %>%
  mutate(treecover_rel00 = treecover_ha/treecover_ha[year == 2000]*100) %>%
  select(c(year, treecover_ha, treecover_rel00)) %>%
...
fig_treeloss_fapbm = ggplot(data = data_plot_fapbm,
  ...
  + labs(x = "", y = "% of 2000 tree cover", 
        ...
        subtitle = paste("Treecover in 2000 :", format(data_plot_fapbm[data_plot_fapbm$year == 2000,]$treecover_ha, digits = 1, big.mark = ",", scientific = F), "ha"),
         caption = "Data : Global Forest Watch") %>%
  + theme(legend.position = "bottom",
      legend.key = element_rect(fill = "white"),
      plot.title = element_text(size = 14, face = "bold"), 
...
fig_treeloss_nofapbm = ggplot(data = data_plot_nofapbm,
  ...
  + labs(x = "", y = "% of 2000 tree cover", 
        ...
        subtitle = paste("Treecover in 2000 :", format(data_plot_nofapbm[data_plot_nofapbm$year == 2000,]$treecover_ha, digits = 1, big.mark = ",", scientific = F), "ha"),
         caption = "Data : Global Forest Watch") %>%
  + theme(legend.position = "bottom",
      legend.key = element_rect(fill = "white"),
      plot.title = element_text(size = 14, face = "bold"), 
...
data_plot_all = data_plot_fapbm %>%
  ...
  left_join(data_plot_mdg, by = "year")
#CARFEUL NEED TO RENAME VARIABLES   

fig_treeloss_all =  ggplot(data_plot_all,
                           aes(x = year)) %>%
  + geom_line(aes(y = treecover_rel00_mdg, color = "mdg")) %>%
  + geom_point(aes(y = treecover_rel00_mdg, color = "mdg")) %>%
  + geom_line(aes(y = treecover_rel00_nofapbm, color = "nofapbm")) %>%
  + geom_point(aes(y = treecover_rel00_nofapbm, color = "nofapbm")) %>%
