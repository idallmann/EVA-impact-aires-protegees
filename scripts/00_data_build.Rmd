...
data_tidy = data_raw_corr %>%
  ...
  # mutate(country_fr = case_when(pays %in% c("P-N N.Caléd", "P-S N.Caléd", "Nlle Caledonie") | iso3 == "NCL" ~ "Nouvelle-Caledonie",
  #                               pays %in% c("Multi-Pays", "Multi-pays") ~ "Multi-pays",
  #                              TRUE ~ country_fr)) %>%
  #   mutate(country_en = case_when(pays %in% c("P-N N.Caléd", "P-S N.Caléd", "Nlle Caledonie")  | iso3 == "NCL"  ~ "New Caledonia",
  #                                 
  #                                 pays %in% c("Multi-Pays", "Multi-pays") ~ "Multi-countries",
  #                                 TRUE ~ country_en)) %>%
  #Some PAs have iso "ZZ" (multi-countries) by funded by Dr Ocean Pacifique. The region is assigned to Oceania
    mutate(region = case_when(is.na(region) == TRUE & libelle_court_direction_regionale == "DR OCEAN PACIFIQUE" ~ "Oceania",
                            TRUE ~ region)) %>%
