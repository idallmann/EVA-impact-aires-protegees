...
data_pa_eva = read_excel("data_raw/BDD_joint.xlsx") %>%
  as.data.frame() %>%
...
data_pa_arb = read_excel("data_raw/BDD_ARB_10082023.xlsx") %>%
  as.data.frame() %>%
  clean_names() %>%
...
data_pa_afd_wdpaid = data_pa_afd %>%
...
#data_pa_afd = read_delim("data_raw/BDD_PA_AFD.csv", delim = ";")
...
data_siop = read_excel("data_raw/BO_AP_16082023.xlsx") %>%
  clean_names() %>%
...
data_raw = data_pa_afd %>%
  ...
  mutate(iso3 = iso3_wdpa,
         iso3 = case_when(is.na(iso3) == TRUE ~ iso3_join,
                          TRUE ~ iso3),
         .after = "iso3_wdpa") 
  

#write_csv(data_raw, "data_raw/BDD_PA_raw.xlsx")
```
...
data_raw = fread("data_raw/BDD_PA_raw.xlsx", encoding = "UTF-8")
...
write_csv(data_raw_corr, "data_raw/BDD_PA_raw_corr.csv")
...
data_raw_corr = fread("data_raw/BDD_PA_raw_corr.csv")

data_tidy = data_raw_corr %>%
  #Select relevant variables
  select(c(libelle_court_direction_regionale,
           pays, pays2, nom_alpha, parent_iso3, iso3.x, iso3.y,
...
list_var_PA_fund = c("id_projet", "nom_du_projet", "nom_ap", "id_concours", 
                "wdpaid", "pays", "iso3", "direction_regionale",
                "cofinanciers", "cofinanciers_siop",
                  "mt_fin_global_af_d_prevu_devise",
                  "montant_prevu_concours_euro_octroi",
                  "mt_global_projet_prevu_devise",
