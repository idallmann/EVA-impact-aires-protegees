...
After manual correction, a tidy dataset is built. 
...
data_nofund_name = data_nofund %>%
  ...
  unique() %>%
  #We keep information on fundings : one row for each name, funding year is kept
...
data_nofund_na = data_nofund %>%
  ...
  mutate(year_funding_all = paste0(year_funding, collapse = ","),
         .after = "year_funding") %>%
  ungroup() %>%
  #Finally we remove duplicated id_project
  group_by(id_projet, iso3) %>%
  slice(1)
