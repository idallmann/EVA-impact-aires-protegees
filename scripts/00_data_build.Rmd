...
# write_delim(data_pa_afd, "data_raw/BDD_PA_AFDtest.csv",
#             delim = ";",
#             na = "NA")
...
data_wdpa$wdpaid=as.double(data_wdpa$wdpaid)
...
#duplicate in x lines the PA in x different countries 
multi_country_row <- data_wdpa %>%
  filter(iso3 == "COG;CMR;CAF") %>%
  slice(rep(1:n(), each = 3)) %>%
  mutate(iso3 = c("COG", "CMR", "CAF"))

data_wdpa <- data_wdpa %>%
  filter(iso3 != "COG;CMR;CAF") %>%
  bind_rows(multi_country_row)
...
## Non-confidential datasetdata_wdpa <- st_read(dsn = "/home/onyxia/work/EVA-impact-aires-protegees/data/WDPA_WDOECM_Jun2024_Public.gdb")
data_pa_nofund_nodupl = 
  #fread("data_raw/BDD_PA_AFD_nofund_nodupl.csv")
  s3read_using(readr::read_delim,
               delim = ";",
...
#Creating the dataset of with geospatial information (polygons)
data_pa_fabpm_shp = data_pa_fapbm %>%
  left_join(data_wdpa, by = c("wdpaid", "wdpa_pid"))
...
pa_shp = 
  ...
  #Select relevant variables
  dplyr::select(c(wdpaid, area_afd_km2, rep_area, geometry, iso3, region_afd, region, sub_region, year_funding_first, year_funding_all)) 

#NO USED ANYMORE BECAUSE 1245 ISNT A AFD FUNDED AP- Spatial definition of wdpaid 555547988 overlaps CMR and CAF. Wdpaid 1245 corresponds to the CMR part. The overlap is removed and iso3 redefined so that 555547988 is CAF only. 
