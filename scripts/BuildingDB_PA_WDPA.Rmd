...
data_PA_tidy = data_PA_raw %>%
  ...
  mutate(across(.cols = c("wdpaid", "montant_prevu_concours_euro_octroi",
...
pa_shp = read_sf("data_raw/WDPA_SHP/BDD_SHP_nodupl.shp") %>%
  clean_names() %>%
  select(c("wdpaid", "name", "desig_e", "iucn", "marin":"gis_a"))
# 
# st_write(pa_shp,
#          dsn = "data_tidy/BDD_shp_tidy.gpkg")
```
