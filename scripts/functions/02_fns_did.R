...
fn_plot_forest_loss_agg = function(df_plot_forest_loss, save_dir, alpha)
{
  ...
  df_avoidefo_agg_all = df_plot_forest_loss_agg_all %>%
    filter(year == 2021) %>%
    group_by(matched) %>%
    summarize(tot_avoidefo_ha = abs(tot_fc_rel00_ha[group == "Control"]) - abs(tot_fc_rel00_ha[group == "Treated"]),
              tot_avoidefo_ha_ciup = abs(tot_fc_rel00_ha_ci_upper[group == "Control"]) - abs(tot_fc_rel00_ha_ci_upper[group == "Treated"]),
              tot_avoidefo_ha_cilow = abs(tot_fc_rel00_ha_ci_lower[group == "Control"]) - abs(tot_fc_rel00_ha_ci_lower[group == "Treated"]),
              avg_avoidefo_ha = abs(avg_fc_rel00_ha[group == "Control"]) - abs(avg_fc_rel00_ha[group == "Treated"]),
              avg_avoidefo_ha_ciup = abs(avg_fc_rel00_ha_ci_upper[group == "Control"]) - abs(avg_fc_rel00_ha_ci_upper[group == "Treated"]),
              avg_avoidefo_ha_cilow = abs(avg_fc_rel00_ha_ci_lower[group == "Control"]) - abs(avg_fc_rel00_ha_ci_lower[group == "Treated"])) %>%
    ungroup() %>%
    mutate(matched = case_when(matched == FALSE ~ "Before matching",
                               matched == TRUE ~ "After matching"
    )) 
  ...
  df_avoidefo_agg_focus = df_plot_forest_loss_agg_focus %>%
    filter(year == 2021) %>%
    group_by(matched) %>%
    summarize(tot_avoidefo_ha = abs(tot_fc_rel00_ha[group == "Control"]) - abs(tot_fc_rel00_ha[group == "Treated"]),
              tot_avoidefo_ha_ciup = abs(tot_fc_rel00_ha_ci_upper[group == "Control"]) - abs(tot_fc_rel00_ha_ci_upper[group == "Treated"]),
  ...
  df_avoidefo_agg_nofocus = df_plot_forest_loss_agg_nofocus %>%
    filter(year == 2021) %>%
    group_by(matched) %>%
    summarize(tot_avoidefo_ha = abs(tot_fc_rel00_ha[group == "Control"]) - abs(tot_fc_rel00_ha[group == "Treated"]),
              tot_avoidefo_ha_ciup = abs(tot_fc_rel00_ha_ci_upper[group == "Control"]) - abs(tot_fc_rel00_ha_ci_upper[group == "Treated"]),
  ...
  fig_forest_loss_agg_avg_all = ggplot(data = filter(df_plot_forest_loss_agg_all, year == year.max),
    ...
    + labs(x = "",
           ...
           subtitle = paste0("Sample : all protected areas (" , n_pa, " areas with ", format(fc_2000_ha_avg_all, big.mark = ","), " ha of forest cover in 2000 on average)"),
           caption = paste(((1-alpha)*100), "% confidence intervals")) %>%
    + facet_wrap(~matched,
                 labeller = labeller(matched = fct.labs))  %>%
    + theme_minimal() %>%
  ...
  fig_avoidefo_agg_tot_all = ggplot(data = df_avoidefo_agg_all,
    ...
    + geom_bar(position =  position_dodge(width = 0.8), stat = "identity", show.legend = FALSE) %>%
    + geom_errorbar(aes(ymax=tot_avoidefo_ha_ciup, ymin=tot_avoidefo_ha_cilow), width=0.3, colour="grey70", alpha=0.9, linewidth=1) %>%
    + geom_label(aes(label = paste("~",format(round(tot_avoidefo_ha, 1), big.mark = ",")), y = tot_avoidefo_ha/2),
                 vjust = -0.5,
                 color = "black",
                 show.legend = FALSE) %>%
    + scale_fill_brewer(name = "Group", palette = "Blues") %>%
  ...
  ### PA of interest
  fig_avoidefo_agg_tot_focus = ggplot(data = df_avoidefo_agg_focus,
                                      aes(y = tot_avoidefo_ha, x = factor(matched, levels = c("Before matching", "After matching")), fill = matched)) %>%
    + geom_bar(position =  position_dodge(width = 0.8), stat = "identity", show.legend = FALSE) %>%
    + geom_errorbar(aes(ymax=tot_avoidefo_ha_ciup, ymin=tot_avoidefo_ha_cilow), width=0.3, colour="grey70", alpha=0.9, linewidth=1) %>%
    + geom_label(aes(label = paste("~",format(round(tot_avoidefo_ha, 1), big.mark = ",")), y = tot_avoidefo_ha/2),
  ...
  ### Other PA
  fig_avoidefo_agg_tot_nofocus = ggplot(data = df_avoidefo_agg_nofocus,
                                        aes(y = tot_avoidefo_ha, x = factor(matched, levels = c("Before matching", "After matching")), fill = matched)) %>%
    + geom_bar(position =  position_dodge(width = 0.8), stat = "identity", show.legend = FALSE) %>%
    + geom_errorbar(aes(ymax=tot_avoidefo_ha_ciup, ymin=tot_avoidefo_ha_cilow), width=0.3, colour="grey70", alpha=0.9, linewidth=1) %>%
    + geom_label(aes(label = paste("~",format(round(tot_avoidefo_ha, 1), big.mark = ",")), y = tot_avoidefo_ha/2),
  ...
  fig_avoidefo_agg_avg_all = ggplot(data = df_avoidefo_agg_all,
    ...
    + geom_bar(position =  position_dodge(width = 0.8), stat = "identity", show.legend = FALSE) %>%
    + geom_errorbar(aes(ymax=avg_avoidefo_ha_ciup, ymin=avg_avoidefo_ha_cilow), width=0.3, colour="grey70", alpha=0.9, linewidth=1) %>%
    + geom_label(aes(label = paste("~",format(round(avg_avoidefo_ha, 1), big.mark = ",")), y = avg_avoidefo_ha/2),
                 vjust = -0.5,
                 color = "black",
                 show.legend = FALSE) %>%
    + scale_fill_brewer(name = "Group", palette = "Blues") %>%
  ...
  ### PA of interest
  fig_avoidefo_agg_avg_focus = ggplot(data = df_avoidefo_agg_focus,
                                      aes(y = avg_avoidefo_ha, x = factor(matched, levels = c("Before matching", "After matching")), fill = matched)) %>%
    + geom_bar(position =  position_dodge(width = 0.8), stat = "identity", show.legend = FALSE) %>%
    + geom_errorbar(aes(ymax=avg_avoidefo_ha_ciup, ymin=avg_avoidefo_ha_cilow), width=0.3, colour="grey70", alpha=0.9, linewidth=1) %>%
    + geom_label(aes(label = paste("~",format(round(avg_avoidefo_ha, 1), big.mark = ",")), y = avg_avoidefo_ha/2),
  ...
  ### Other PA
  fig_avoidefo_agg_avg_nofocus = ggplot(data = df_avoidefo_agg_nofocus,
                                        aes(y = avg_avoidefo_ha, x = factor(matched, levels = c("Before matching", "After matching")), fill = matched)) %>%
    + geom_bar(position =  position_dodge(width = 0.8), stat = "identity", show.legend = FALSE) %>%
    + geom_errorbar(aes(ymax=avg_avoidefo_ha_ciup, ymin=avg_avoidefo_ha_cilow), width=0.3, colour="grey70", alpha=0.9, linewidth=1) %>%
    + geom_label(aes(label = paste("~",format(round(avg_avoidefo_ha, 1), big.mark = ",")), y = avg_avoidefo_ha/2),
  ...
  ggsave(paste(tmp, paste0("fig_fl_2000_", year.max, "_agg_tot_all.png"), sep = "/"),
         plot = fig_forest_loss_agg_tot_all,
         device = "png",
         height = 6, width = 9)
  ...
  ggsave(paste(tmp, paste0("fig_fl_2000_", year.max, "_agg_avg_all.png"), sep = "/"),
         plot = fig_forest_loss_agg_avg_all,
         device = "png",
         height = 6, width = 9)
  ...
  ggsave(paste(tmp, paste0("fig_avoidefo_2000_", year.max, "_agg_tot_all.png"), sep = "/"),
         plot = fig_avoidefo_agg_tot_all,
         device = "png",
         height = 6, width = 9)
  ggsave(paste(tmp, paste0("fig_avoidefo_2000_", year.max, "_agg_tot_focus.png"), sep = "/"),
         plot = fig_avoidefo_agg_tot_focus,
         device = "png",
         height = 6, width = 9)
  ggsave(paste(tmp, paste0("fig_avoidefo_2000_", year.max, "_agg_tot_nofocus.png"), sep = "/"),
         plot = fig_avoidefo_agg_tot_nofocus,
         device = "png",
         height = 6, width = 9)
  
  ggsave(paste(tmp, paste0("fig_avoidefo_2000_", year.max, "_agg_avg_all.png"), sep = "/"),
         plot = fig_avoidefo_agg_avg_all,
         device = "png",
         height = 6, width = 9)
  ggsave(paste(tmp, paste0("fig_avoidefo_2000_", year.max, "_agg_avg_focus.png"), sep = "/"),
         plot = fig_avoidefo_agg_avg_focus,
         device = "png",
         height = 6, width = 9)
  ggsave(paste(tmp, paste0("fig_avoidefo_2000_", year.max, "_agg_avg_nofocus.png"), sep = "/"),
         plot = fig_avoidefo_agg_avg_nofocus,
         device = "png",
         height = 6, width = 9)
  ...
  for(f in files) 
  {
    cat("Uploading file", paste0("'", f, "'"), "\n")
    aws.s3::put_object(file = f, 
                       bucket = paste("projet-afd-eva-ap", save_dir, "forest_loss_agg", sep = "/"),
                       region = "", show_progress = TRUE)
  }
...
fn_plot_att_afd = function(df_fc_att, df_fl_att, list_pa_focus, list_iso_focus, alpha, save_dir)
{
  ...
  for (i in list_iso_focus)
  {
    ...
    cat("Uploading file", paste0(tmp, "/fig_att_per_focus_others_", i, ".png"), "\n")
  ...
  for (i in list_iso_focus)
  {
    ...
    cat("Uploading file", paste0(tmp, "/fig_att_pa_focus_others_", i, ".png"), "\n")
  ...
  tbl_fc_att_per = df_fc_att %>%
    ...
    dplyr::select(c(name_pa, focus, id_projet, country_en, treatment_year, funding_year_list, kfw, ffem, iucn_wolf, gov_type, time, att_per, sig_per)) %>%
    filter(time %in% c(5, 10)) %>%
    pivot_wider(values_from = c("att_per", "sig_per"), names_from = c("time", "time")) %>%
    dplyr::select(c(name_pa, focus, id_projet, country_en, treatment_year, funding_year_list, kfw, ffem, iucn_wolf, att_per_5, sig_per_5, att_per_10, sig_per_10)) %>%
    #dplyr::select(c(name_pa, id_projet, country_en, treatment_year, funding_year_list, kfw, ffem, iucn_wolf, gov_type, att_per_5, sig_per_5, att_per_10, sig_per_10)) %>%
    mutate(across(.cols = starts_with(c("att", "sig")),
                  .fns = \(x) case_when(is.na(x) == TRUE ~ "/", TRUE ~ as.character(format(x, scientific = F, digits = 1))))) %>%
    mutate(across(.cols = c(id_projet, kfw, ffem),
  ...
  tbl_fc_att_pa = df_fc_att %>%
    ...
    dplyr::select(c(name_pa, focus, id_projet, country_en, treatment_year, funding_year_list, kfw, ffem, iucn_wolf, gov_type, time, att_pa, sig_pa)) %>%
    filter(time %in% c(5, 10)) %>%
    ...
    dplyr::select(c(name_pa, focus, id_projet,  country_en, treatment_year, funding_year_list, kfw, ffem, iucn_wolf, att_pa_5, sig_pa_5, att_pa_10, sig_pa_10)) %>%
    mutate(across(.cols = starts_with(c("att", "sig")),
                  .fns = \(x) case_when(is.na(x) == TRUE ~ "/", TRUE ~ as.character(format(x, scientific = F, digits = 1))))) %>%
    mutate(across(.cols = c(id_projet, kfw, ffem),
                  .fns = \(x) case_when(is.na(x) == TRUE ~ "", TRUE ~ x))) %>%
  ...
  names(tbl_fc_att_pa) = c("Name", "Interest", "Proj. ID", "Country", "Creation", "Funding year", "KfW", "FFEM", "Protection",
  ...
  tbl_fl_att = df_fl_att %>%
    ...
    dplyr::select(c(name_pa, focus, id_projet, country_en, treatment_year, funding_year_list, kfw, ffem, iucn_wolf, gov_type, time, att, sig)) %>%
    ...
    dplyr::select(c(name_pa, focus, id_projet,  country_en, treatment_year, funding_year_list, kfw, ffem, iucn_wolf, att_5, sig_5, att_10, sig_10)) %>%
  ...
  ggsave(paste(tmp, "fig_att_per_other.png", sep = "/"),
         plot = fig_att_per_other,
         device = "png",
         height = 17, width = 20)
  ...
  ggsave(paste(tmp, "fig_att_pa_other.png", sep = "/"),
         plot = fig_att_pa_other,
         device = "png",
         height = 17, width = 20)
  ...
  ggsave(paste(tmp, "fig_att_fl_other.png", sep = "/"),
         plot = fig_att_fl_other,
         device = "png",
         height = 17, width = 20)
...
fn_plot_att_agg = function(df_fc_att, df_fl_att, is_focus, alpha, save_dir)
{
  ...
  #Number of PA
  n_pa = length(unique(df_fc_att$wdpaid))
  ...
  for(f in files) 
  {
    cat("Uploading file", paste0("'", f, "'"), "\n")
    aws.s3::put_object(file = f, 
                       bucket = ifelse(is_focus == T, 
                                       yes = paste("projet-afd-eva-ap", save_dir, "att_agg", "focus", sep = "/"),
                                       no = paste("projet-afd-eva-ap", save_dir, "att_agg", "nofocus", sep = "/")),
                       region = "", show_progress = TRUE)
  }
  do.call(file.remove, list(list.files(tmp, full.names = TRUE)))
