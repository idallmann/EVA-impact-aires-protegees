...
fn_pre_group = function(iso, wdpa_raw, status, yr_min, path_tmp, utm_code, buffer_m, data_pa, gadm_prj, grid, gridSize, log, save_dir)
{
  ...
  output = withCallingHandlers(
    
    {
      ...
      ##PAs in the sample which can bu used in impact evaluation : in the country of interest, wdpaid known, area above 1km² (Wolf et al. 2021), implemented after yr_min defined by the user, non-marine (terrestrial or coastal, Wolf et al. 2021)
      ### List of PA compatiable with IE
      pa_sample_ie = data_pa %>%
        filter(iso3 == iso & is.na(wdpaid) == FALSE & area_km2 > 1 & status_yr >= yr_min & marine %in% c(0,1))
      ### Then we need to remove any overlap between these PA, to avoid risk of double treatment we could not disentangle a priori.
      wdpa_prj_ie_vect = wdpa_prj %>% 
        filter(iso3 == iso & is.na(wdpaid) == FALSE & area_km2 > 1 & status_yr >= yr_min & marine %in% c(0,1)) %>% #Select PA which can be IE
        vect() #Convert to SpatialVector to use convenient functions of the terra package
      #### The polygon layer without any overlap, the area of the polygon without overlap, and a dummy (if the area without overlap is below 10% of the initial area, we can discard the area)
      wdpa_prj_ie_noverlap = sapply(1:dim(wdpa_prj_ie_vect)[1], 
                                    function(X) {erase(wdpa_prj_ie_vect[X,], wdpa_prj_ie_vect[-X,])}) %>%
        vect() %>%
        st_as_sf() %>%
        mutate(area_nover_km2 = expanse(vect(geometry), unit = "km"),
               rm = area_nover_km2 < 0.1*area_km2)
      ### The layer of intersections : the intersected areas must not be used as control
      wdpa_prj_ie_overlap = sapply(1:dim(wdpa_prj_ie_vect)[1], 
                                   function(X) {intersect(wdpa_prj_ie_vect[X,], wdpa_prj_ie_vect[-X,])}) %>%
        vect() %>%
        st_as_sf()
      ...
      ## During rasterization, in case a cell of the raster is on both sample analysed and not in sample, we want the cell to take the WDPAID of the funded analysed.
      ## Same for sample, not analyzed. As the first layer is taken, wdpa_sample_ie needs to be first !
      ...
      # Rasterize wdpaid
      ## CAREFUL : as stated above, the wdpa_groups raster is ordered so that the first layer is the one of sample, analyzed PA. Thus one needs to have fun = "first"
      ...
      #/!\ For the moment, a pixel both in and out of the sample is considered in the sample !
      #But if sample not analyzed AND sample analyzed, then sample not analyzed.
      #Idea : the pixel could be treated out of the period considered, so not comparable to other treatment pixels considered in sample, analyzed.
