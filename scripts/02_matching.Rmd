...
#list_iso = c("COG", "MDG", "MOZ", "NER", "TCD")
list_iso = c("COM", "STP", "GNB", "GMB", "SEN", "MAR", "GAB", "KEN", "CMR", "COG", "MDG")
```
...
df_no_match = data.frame(wdpaid = NULL) #To report the PA where matching was not possible
...
for (i in list_iso)
{
  ...
  for (j in list_pa)
  {
    ...
    output_cem = fn_post_match_auto(mf = mf_j, iso = i, 
                                   ...
                                   log = log)
    if(output_cem$is_ok == FALSE) 
      {
      df_no_match = rbind(df_no_match, data.frame(wdpaid = j)) #Record the unmatched PA
      next #Go to next PA or country
      } else out_cem_j = output_cem$out.cem
    
    ##Add matching quality metrics of the PA matched in this iteration
    tbl.quality.j = output_cem$tbl.quality
    df_quality_ct = rbind(df_quality_ct, tbl.quality.j) 
    
    #Plots : covariates
    print("--Some plots : covariates")
    print("----Covariate balance")
    output_covbal = fn_post_covbal(out.cem = out_cem_j,
                                   tbl.quality = tbl.quality.j,
                   mf = mf_j,
                   colname.travelTime = colname.travelTime, 
                   colname.clayContent = colname.clayContent,
                   colname.fcAvg = colname.fcAvg, 
  ...
  if(output_covbal$is_ok == FALSE) {next}
    ...
    ##Add matching quality metrics of the PA matched in this iteration
    df_quality_tt = rbind(df_quality_tt, output_m_unm_treated$tbl.quality)
...
#Finally save the dataframe on matching quality
