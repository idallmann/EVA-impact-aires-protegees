...
list_iso = unique(data_pa[data_pa$direction_regionale == "Dr Afrique Centrale", ]$iso3)
...
log = fn_pre_log(list_iso,
                 name = paste0("log-", Sys.Date(), "-test-LAO+GIN.txt"),
                 notes = "All country in Dr Ocean Indien")
...
for (i in list_iso)
{
  ...
  output_pre_grid = fn_pre_grid(iso = i, 
                                ...
                                sampling = n_sampling,
                                log = log)
  if(output_pre_grid$is_ok == FALSE) {next}
  ...
  output_pre_group = fn_pre_group(iso = i, wdpa_raw = wdpa_wld_raw,
                            yr_min = yr_min,
                            ...
                            gridSize = gridSize,
                            log = log)
  if(output_pre_group$is_ok == FALSE) {next} else grid_param = output_pre_group$grid.param
...
aws.s3::put_object(file = log,
                   bucket = "projet-afd-eva-ap/data_tidy/mapme_bio_data/matching",
                   region = "",
                   show_progress = FALSE)
