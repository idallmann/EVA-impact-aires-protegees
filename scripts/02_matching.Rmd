...
for (i in list_iso)
{
  ...
  for (j in list_pa)
  {
    ...
    #                    colname.fcIni = colname.fcIni, 
    #                    colname.flAvg = colname.flAvg)
    # if(out.cem_j == "skip_to_next") {next}
    
    out.cem_j = fn_post_match(mf = mf_j, iso = i, path_tmp = tmp_post,
                       colname.travelTime = colname.travelTime, 
                       colname.clayContent = colname.clayContent,
                       colname.fcIni = colname.fcIni, 
                       colname.flAvg = colname.flAvg)
    
    test = summary(out.cem_j, interactions = TRUE)
    nn = test$nn %>% as.data.frame()
    sum.all = test$sum.all %>% as.data.frame()
    sum.matched = test$sum.matched %>% as.data.frame()
    test = out.cem_j$info
    plot.matchit(out.cem_j)
    plot.summary.matchit(out.cem_j)
    #Plots : covariates
