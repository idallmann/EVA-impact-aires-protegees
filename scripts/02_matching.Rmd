...
for (i in list_iso)
{
  ...
  # tic_parallel = tic()
  # fn_pre_mf_parallel_test(grid.param = grid_param, path_tmp = tmp_pre, iso = i,
...
get.soil_np1 = get_resources(aoi,
...
#Parallel
  ...
  plan(multisession, workers = availableCores()) # set up parallel plan with 6 concurrent threads

  tic_p1 = tic()

  dl.soil_p1 = get_resources(aoi,
...
calc.soil_p1 %<-% calc_indicators(dl.soil_p1,
...
get.soil_p1 = unnest(calc.soil_p1, soilproperties) %>%
...
get.soil_np2 = get_resources(aoi,
...
get.travelT_np2 = get_resources(aoi, resources = "nelson_et_al",
...
#Parallel
  ...
  dl.travelT_p2 = get_resources(aoi, resources = "nelson_et_al",
...
calc.soil_p2 %<-% calc_indicators(dl.soil_p2,
...
calc.travelT_p2 %<-% calc_indicators(dl.travelT_p2, 
...
get.soil_p2 = unnest(calc.soil_p2, soilproperties) %>%

  mutate(across(mean, round, 3)) %>% # Round numeric columns

  pivot_wider(names_from = c("layer", "depth", "stat"), values_from = "mean")

get.travelT_p2 = unnest(calc.travelT_p2, traveltime) %>%
...
get.soil_np3 = get_resources(aoi,
...
get.travelT_np3 = get_resources(aoi, resources = "nelson_et_al",
...
get.tree_np3 = get_resources(aoi, resources = c("gfw_treecover", "gfw_lossyear")) %>%
...
#Parallel
  ...
  dl.travelT_p3 = get_resources(aoi, resources = "nelson_et_al",

                            range_traveltime = c("5k_110mio"))

  

  dl.tree_p3 = get_resources(aoi, resources = c("gfw_treecover", "gfw_lossyear"))
...
calc.soil_p3 %<-% calc_indicators(dl.soil_p3,
...
calc.travelT_p3 %<-% calc_indicators(dl.travelT_p3, 
...
calc.tree_p3 %<-% calc_indicators(dl.tree_p3,
...
get.soil_p3 = unnest(calc.soil_p3, soilproperties) %>%

  mutate(across(mean, round, 3)) %>% # Round numeric columns

  pivot_wider(names_from = c("layer", "depth", "stat"), values_from = "mean")

get.travelT_p3 = unnest(calc.travelT_p3, traveltime) %>%

  pivot_wider(names_from = "distance", values_from = "minutes_median", names_prefix = "minutes_median_")

get.tree_p3 = unnest(calc.tree_p3, treecover_area) %>%
