...
data_pa_ie_africa = data_pa %>%
  filter(region == "Africa" & is.na(wdpaid) == FALSE & area_km2 >=1 & marine %in% c(0,1) & status_yr >= yr_min & afd == T)
...
Test to remove overlap between PAs and take the intersection as a "non-control category"

```{r}
install.packages("mapview")


wdpa_vect = wdpa_wld_raw %>%
  clean_names() %>%
  filter(iso3 == "GAB" & st_geometry_type(geom) == "MULTIPOLYGON") %>%
  st_make_valid() %>%
  st_cast("POLYGON") %>%
  vect()

wdpa_noverlap = sapply(1:dim(wdpa_vect)[1], 
                                 function(X) {erase(wdpa_vect[X,], wdpa_vect[-X,])}) %>% 
  vect() %>%
  st_as_sf() %>%
...
wdpa_overlap = sapply(1:dim(wdpa_vect)[1], 
                                 function(X) {intersect(wdpa_vect[X,], wdpa_vect[-X,])}) %>% 
  vect() %>%
  st_as_sf() 
```
