...
:   2.2 Obtain information about `gadm` and `gadm_prj` object. To do so
    you can just enter the name of the object in the console.
...
:   3.1 Find the right *sf* function to create buffer around all PA
    using the buffer size (`buffer_m`) chosen before and complete the
    code
...
wdpa.sub = wdpa_groups %>%
  st_intersects(gadm_prj, .) %>%
  unlist()
# Filter the PA and the buffer to the subset
...
# Specify the raster resolution equal to the pre-defined grid size ('gridSize')
res(r.ini) = gridSize
...
:   3.2 Try to change the function used in `exact_extract()` to retain
    the smallest pixel value.
...
:   5.1 Run the following code and comment on the mean distance to city
    in El Salvador
