...
methodology used. The exercises conduct an evaluation for a single PA
(Parque Nacional Montecristo in El Salvador) to understand the code
easier, but the scripts to analyze an entire portfolio can be found here
'github link'.
...
-   Create a shiny app to visualize the data interactively.
...
1.2The next step is to decide the parameters of your analysis, in the
...
To lower the computation time we choose to analyze a PA in a small
country, namely the Parque Nacional Montecristo, in Salvador.

However, after you have done this exercise, you can try it with a
different PA. It is also possible to vary the following parameters
according to the PA you wish to analyze. You can start using the same
parameters as in the case study. Afterward you could run the notebook
again with different parameters to see the differences in the results.
...
```{r}
# Download country polygon to the working directory and load it into the workspace 
...
# Make a bounding box around the reprojected country polygon (because it is required in the next function to build the grid )

bbox = st_bbox(gadm_prj) %>% st_as_sfc() %>% st_as_sf()
# Grid to the extent of the bounding box
grid.ini = st_make_grid(bbox, cellsize = c(gridSize,gridSize))

# Crop the Grid to the extent of country boundary by
...
2.2 Obtain information about `gadm` and `gadm_prj` object. To do so you
...
# PA polygon are downloaded to the working directory.
...
# Separate the analyzed PA from others PAs
...
# Merge the dataframes of Treatment candidate, other PAs and buffer zone
...
The next step is to assign a group ( PA analyzed, other PAs,
Buffer,others ) to each cell of the grid. It may be the case that a cell
is covered by multiple groups, so it is necessary to decide how the
group of the cell will be attributed. To achieve this, we must first
rasterize wdpa data, which involves transforming it from a vector
representation to a raster grid. Using raster data is a more
time-efficient than vector, which is especially beneficial in a
portfolio analysis where a significant amount of data must be processed.
...
# Specify the raster resolution equal to the pre-defined 'gridSize'
res(r.ini) = gridSize
# Assign the raster cells with "Group" values,
# Take the minimal value if a pixel is covered by overlapping polygons, so that PA Groups (1 and 2) have higher priority than Buffer group.
...
3.2 Try to change the function used in `exact_extract()` to retain the
smallest pixel value.

3.3 Compute group statistics of grid.param and grid.param_2, comment the
difference
...
:   **Multisession** refers to a type of parallel processing where
    ...
    To learn more about multisession see this exercise **(ADD A LINK TO
    ANOTHER EXERCISE OR CHAPTER)**
...
4.2 Try running the same code chunk without the multisession and compare
execution times
...
4.3 Insert the function to calculate elevation mean, you should use the
mean statistic and the exactextract engine :
...
4.4 Download data related to travel time and calculate the median travel
time
