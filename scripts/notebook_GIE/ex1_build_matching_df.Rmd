...
The following exercises will help you master the steps of the
...
The objective of this first exercise is to build for a given PA a dataset with outcomes, covariates and treatment variables for each unit of observation (cells) that will then be used in the matching process . This notebook will also enable you to create your own shiny app to view the data interactively. 
...
## 1. Settings ⚙️
Please enter a working directory and the name you wish to give to the output of this notebook, which is a dataset ready for matching : 
...
It is also possible to vary the following parameters according to the PA you wish to analyse. Initially you can use the same parameters as in the case study. You can then run the notebook again with different parameters to see the difference in the results.
...
is generated within this box so that it covers the whole country. It is then intersected with the country's boundary to retain only cells located within the country 
...
2.Obtain information about "gadm" and "gadm_prj" object. To do so you can just enter the name of the object in the console. 
...
Then, we need to define the status of each observation unit (cell), meaning whether it is concerned by the analysed PA (group 1), an other PA (group 2), or a buffer area around a PA (group 3). For each cell, we also need to retrieve the WDPA ID, when applicable. Cells belonging to none of those groups form the group of potential control units (group 0). 
To this end this section first downloads protected area (PA) polygons from WDPA and reprojects them to the same projection used previously. After some filtering and cleaning of PA polygons, a buffer is created around each one of them. Then, each polygon is assigned to a group (group 1 for the analysed PA, group 2 for other Pas and group 3 for buffer zones).
In a second step, this dataset of PA polygons and buffers is rasterized, and zonal statistics are used to define the treatment group of each cell/observation unit. 
...
The objective is to define to which group each cell of the grid belongs to (analysed PA, other PA, buffer, none).To speed
...
A raster layer is created over the country's extent, with pixel size equal to our grid cell size. 
The polygon layer (containing PAs and buffer zones) is rasterized, and each pixel in the newly created raster is assigned to a group with pixel
values representing group IDs. When a pixel is covered by overlapping polygons the pixel value is assignedthe smallest group value. 
For instance, if a pixel is located both in a buffer polygon and in the analysed P, the pixel value will be set to group 1 i.e. the analyzed PA zone, ensuring that PA group IDs take precedence over buffer IDs
 Another raster layer is created to assign a WDPA id to each unit of observation.
IDs.
...
You can now visualize the gridding of the country and the attribution of a
group to each cell.
...
The function used to aggregate pixel values into the grid cells in the exact_extract() function influences the final group attribution obtained for each cells 

Instead of assigning to a grid cell the most frequent value among the pixels it overlaps we may want to retain the smallest pixel value, so that if the grid cell overlaps with any pixels that intersects a PA, the grid cell is assigned to the PA group. 
1. Go back to the previous code chunk and try to change the function used in exact_extract() to retain the smallest pixel value.
...
Once each cell has been assigned to a group, all the covariates need
to be downloaded. To do so you can use the Mapme.biodiversity package
that enables you to download multiple resources and then compute the
...
2.  Try running the same code chunk without the multisession and compare execution times
