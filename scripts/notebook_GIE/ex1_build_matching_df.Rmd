...
-   Find the right sf function to create buffer around all PA using the
    buffer size (buffer_m) choose before and complete the code
...
-   change the function of extract extact to a min or a max and comment
    the result
...
Try to change the function used by exactextract to "min", then to
understand the difference compute group statistics of grid.param and map
...
# Divise graphical space in two columns
par(mfrow=c(1, 2)) 
...
# Remettre les paramètres graphiques à la normale
par(mfrow=c(1, 1))
```
...
-add a question on multisession + function on time
...
Enter the function to download soil data. Use the following parameters :
"clay' for the layers , a depth between 0 and 5 cm, and the statistic
"mean".
