...
question on projection, different projecttion exist, need to use the right one 
add the right objecti to obtain the map of the grid and the country 
...
# This part will be remove, but the wdpa_fetch is not working for now 
...
Then a a raster grid is initialized over the country's
...
- explain why se're using a raster (Ask Melvin)
- change the function of extract extact to a min or a max and comment the result 
...
function to complete the following code. You will be ask to : 
